				...
				if (e.getButton() == 1 && e.getSource() == arrayButton[i][j] && !world.getArrayCamCo()[i][j]) {
					...
					if (!world.open(i, j)) {
						...
						if (world.isComplete()) {
							...
							int option = JOptionPane.showConfirmDialog(this, "Bạn đã chọn ô chứa mìn -> Lose :((", "Notification",
							...
							int option = JOptionPane.showConfirmDialog(this, "Bạn đã thắng, chơi lại hong nè :v", "Notification",
									JOptionPane.YES_NO_OPTION);
							if (option == JOptionPane.YES_OPTION) {
								gameFrame.setVisible(false);
								new GameFrame(w, h, boom);
